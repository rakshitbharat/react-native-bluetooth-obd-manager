 ...
 */
export const addDeviceToHistory = async (device: Peripheral, isOBDDevice = true): Promise<void> => {
  try {
...
export const getRecentDevices = async (limit = 5): Promise<DeviceHistoryEntry[]> => {
  try {
    ...
    return history.sort((a, b) => b.lastConnected - a.lastConnected).slice(0, limit);
  } catch (error) {
