...
export async function findServiceAndCharacteristic(
  ...
  try {
    ...
    const service = peripheralInfo.services.find(s => {
      ...
      return SERVICE_UUIDS.some(
        serviceId =>
          uuid.includes(serviceId) || uuid === `0000${serviceId}-0000-1000-8000-00805f9b34fb`,
      );
    });
 ...
 */
function findWriteCharacteristic(characteristics: Characteristic[]): Characteristic | null {
  // First try to find a characteristic with write without response (preferred)
  const writeWithoutResponse = characteristics.find(
    c => c.properties?.WriteWithoutResponse === 'WriteWithoutResponse',
  );
  ...
  const writeWithResponse = characteristics.find(c => c.properties?.Write === 'Write');
  ...
  const anyWriteCharacteristic = characteristics.find(c => {
    const props = c.properties || {};
    return Object.keys(props).some(key => key.toLowerCase().includes('write'));
  });

  return anyWriteCharacteristic || null; // Explicitly return null if nothing found
}

/**
 * Find the best notify characteristic for OBD communication
 */
function findNotifyCharacteristic(characteristics: Characteristic[]): Characteristic | null {
  // First try to find a characteristic with notify property
  const notifyChar = characteristics.find(c => c.properties?.Notify === 'Notify');
  ...
  const commonNotifyChar = characteristics.find(c => {
    ...
    return charUUID.includes('ffe1') || charUUID.includes('fff1') || charUUID.includes('fff2');
  });
 ...
 */
function shouldUseWriteWithResponse(characteristic: Characteristic): boolean {
