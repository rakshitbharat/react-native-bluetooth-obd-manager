...
export const STANDARD_PIDS = {
  ...
  SUPPORTED_PIDS_C1_E0: {
    pid: 'C0',
    description: 'Supported PIDs [C1-E0]',
    mode: 1,
    bytes: 4,
    formula: 'N/A',
  },
 ...
 */
export const convertPidValue = (data: string, command: string): number | string | null => {
  try {
    if (!data || !command) {
      return null;
    }
...
export const parseDTCResponse = (dtcResponse: string): string[] => {
  try {
    ...
    for (let i = 0; i < bytes.length; i += 2) {
      ...
      const suffix = (((firstByte & 0x3f) << 8) | secondByte)
        .toString(16)
        .toUpperCase()
        .padStart(3, '0');
