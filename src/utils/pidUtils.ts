...
export const isPIDSupported = (bitmap: string, pid: number): boolean => {
  if (!bitmap || !/^[0-9A-Fa-f]+$/.test(bitmap)) {
    throw new BluetoothOBDError(BluetoothErrorType.INVALID_PARAMETER, 'Invalid bitmap format');
  }
...
export const getResponseValue = (response: string, mode: number, pid: number): string => {
  ...
  const expectedHeader = `${(mode + 0x40).toString(16).padStart(2, '0')}${pid
    .toString(16)
    .padStart(2, '0')}`;
 ...
 */
export const convertPIDValue = (value: string, mode: number, pid: number): number | null => {
  try {
