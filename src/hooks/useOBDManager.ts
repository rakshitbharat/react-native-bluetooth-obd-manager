...
export const useOBDManager = ({
  ...
  const initializeOBD = useCallback(async () => {
    ...
    try {
      ...
      logOBDDataError('Adapter version:', new Error(version));
      ...
      try {
        ...
        if (pids01 && pids01.length > 0) {
          ...
          logOBDDataError('Supported PIDs 01-20:', new Error(pids01));
      ...
      try {
        ...
        // VIN response needs special parsing
        logOBDDataError('VIN response:', new Error(vinResponse));
      ...
      try {
        ...
        if (success) {
          ...
          try {
            ...
            logBluetoothError(
              'Failed to initialize OBD device',
              error instanceof Error ? error.message : String(error),
            );
            return false;
          }
