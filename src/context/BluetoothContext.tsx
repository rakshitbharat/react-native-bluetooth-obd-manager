...
import { BluetoothState, BluetoothDeviceInfo } from '../types/bluetoothTypes';
...
export interface BluetoothContextType extends BluetoothState {
  ...
  isConnected: boolean;
  getRecentDevices: () => Promise<BluetoothDeviceInfo[]>;
...
export const BluetoothProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const sendCommand = useCallback(
    async (command: string, timeoutMs = 5000): Promise<{ text: string; bytes: number[] }> => {
  ...
  ); // Remove isStreaming as it's not used in the callback
  ...
  useEffect(() => {
    if (connectedDevice) {
      ...
      try {
        ...
        addDeviceToHistory(peripheralDevice, isOBDDevice(peripheralDevice));
