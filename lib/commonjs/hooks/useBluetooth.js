...
var _BluetoothContext = require("../context/BluetoothContext");
...
const handleError = error => {
  ...
  if (typeof error === 'object' && error !== null && 'errorCode' in error && 'message' in error) {
    return new Error(`BLE Error (${error.errorCode}): ${error.message}`);
  }
  return new Error(String(error));
};

// Fix error handling for null cases
const handleStateError = error => {
  if (!error) {
 ...
 */
const useBluetooth = () => {
  const state = (0, _BluetoothContext.useBluetoothState)();
  const dispatch = (0, _BluetoothContext.useBluetoothDispatch)();
  const {
    currentCommandRef
  } = (0, _BluetoothProvider.useInternalCommandControl)();

  // Refs to manage promises for ongoing async operations initiated by the hook
  const scanPromiseRef = (0, _react.useRef)(null);
  const connectPromiseRef = (0, _react.useRef)(null);
  const disconnectPromiseRef = (0, _react.useRef)(null);
  ...
  const checkPermissions = (0, _react.useCallback)(async () => {
    ...
    if (_reactNative.Platform.OS === 'android') {
      if (_reactNative.Platform.Version >= 31) {
        // Android 12+
        requiredPermissions = [Permissions.PERMISSIONS.ANDROID.BLUETOOTH_SCAN, Permissions.PERMISSIONS.ANDROID.BLUETOOTH_CONNECT, Permissions.PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION // Still recommended for reliable scanning
        ];
      } else {
        // Android 11 and below
        requiredPermissions = [Permissions.PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION // Required for BLE scanning
        ];
      }
    ...
    if (requiredPermissions.length === 0) {
      ...
      dispatch({
        ...
        payload: true
      }); // Assume true if none needed
      return true;
    }
    try {
      const statuses = await Permissions.checkMultiple(requiredPermissions);
      const allGranted = requiredPermissions.every(permission => statuses[permission] === Permissions.RESULTS.GRANTED);
   ...
   * @returns {Promise<boolean>} True if all required permissions are granted, false otherwise
   */
  const requestBluetoothPermissions = (0, _react.useCallback)(async () => {
    ...
    if (_reactNative.Platform.OS === 'android') {
      if (_reactNative.Platform.Version >= 31) {
        // Android 12+
        permissionsToRequest = [Permissions.PERMISSIONS.ANDROID.BLUETOOTH_SCAN, Permissions.PERMISSIONS.ANDROID.BLUETOOTH_CONNECT, Permissions.PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION];
      } else {
        // Android 11 and below
        permissionsToRequest = [Permissions.PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION];
      }
    } else if (_reactNative.Platform.OS === 'ios') {
    ...
    try {
      ...
      if (permissionsToRequest.length > 0) {
        ...
        allGranted = permissionsToRequest.every(permission => statuses[permission] === Permissions.RESULTS.GRANTED);
      ...
      if (_reactNative.Platform.OS === 'ios' && iosBlePermissionNeeded) {
        ...
        iosBluetoothGranted = bleStatus === Permissions.RESULTS.GRANTED || bleStatus === Permissions.RESULTS.UNAVAILABLE;
      ...
      dispatch({
        type: 'SET_PERMISSIONS_STATUS',
        payload: finalGranted
      });
      if (!finalGranted) {
        console.warn('[useBluetooth] Not all required permissions were granted.');
        const blocked = Object.values(finalStatuses).some(status => status === Permissions.RESULTS.BLOCKED);
        if (blocked) {
   ...
   */
  const promptEnableBluetooth = (0, _react.useCallback)(async () => {
    ...
    if (_reactNative.Platform.OS === 'android') {
      try {
        ...
        await _reactNativeBleManager.default.enableBluetooth();
        ...
        const formattedError = handleError(error);
        console.error('[useBluetooth] Failed to request Bluetooth enable (e.g., user denied):', formattedError);
        throw new Error(`Failed to enable Bluetooth: ${formattedError.message}`);
      }
    } else if (_reactNative.Platform.OS === 'ios') {
   ...
   */
  const scanDevices = (0, _react.useCallback)(async (scanDurationMs = 5000) => {
    var _scanPromiseRef$curre2;
    if (!state.isBluetoothOn) {
    ...
    if (state.isScanning) {
      ...
      return ((_scanPromiseRef$curre = scanPromiseRef.current) === null || _scanPromiseRef$curre === void 0 ? void 0 : _scanPromiseRef$curre.promise) ?? Promise.resolve();
    }
    ...
    scanPromiseRef.current = createDeferredPromise();
    try {
      ...
      const timeoutId = setTimeout(async () => {
        if (state.isScanning) {
          ...
          try {
            ...
            await _reactNativeBleManager.default.stopScan();
          } catch (stopError) {
      ...
      // Start scan
      await _reactNativeBleManager.default.scan([], scanSeconds, false);
    ...
    return (_scanPromiseRef$curre2 = scanPromiseRef.current) === null || _scanPromiseRef$curre2 === void 0 ? void 0 : _scanPromiseRef$curre2.promise;
  }, [state.isBluetoothOn, state.hasPermissions, state.isScanning, state.error, dispatch]);
   ...
   */
  const connectToDevice = (0, _react.useCallback)(async deviceId => {
    ...
    connectPromiseRef.current = createDeferredPromise();
    try {
      var _peripheralInfo$servi;
      // --- Internal Connection Logic ---
      // 1. Connect
      await _reactNativeBleManager.default.connect(deviceId);
      console.info(`[useBluetooth] Peripheral ${deviceId} connected. Retrieving services...`);

      // 2. Retrieve Services & Characteristics
      const peripheralInfo = await _reactNativeBleManager.default.retrieveServices(deviceId);
      console.info(`[useBluetooth] Services retrieved for ${deviceId}. Found:`, (_peripheralInfo$servi = peripheralInfo.services) === null || _peripheralInfo$servi === void 0 ? void 0 : _peripheralInfo$servi.map(s => s.uuid));
      ...
      for (const target of _constants.KNOWN_ELM327_TARGETS) {
        var _peripheralInfo$servi2;
        ...
        const foundService = (_peripheralInfo$servi2 = peripheralInfo.services) === null || _peripheralInfo$servi2 === void 0 ? void 0 : _peripheralInfo$servi2.find(s => s.uuid.toUpperCase() === serviceUUIDUpper);
        if (foundService) {
          var _peripheralInfo$chara;
          ...
          const characteristics = ((_peripheralInfo$chara = peripheralInfo.characteristics) === null || _peripheralInfo$chara === void 0 ? void 0 : _peripheralInfo$chara.filter(c => c.service.toUpperCase() === serviceUUIDUpper)) ?? [];
          const writeCharacteristic = characteristics.find(c => c.characteristic.toUpperCase() === writeCharUUIDUpper);
          const notifyCharacteristic = characteristics.find(c => c.characteristic.toUpperCase() === notifyCharUUIDUpper);
          if (writeCharacteristic && notifyCharacteristic) {
            ...
            let writeType = 'WriteWithoutResponse'; // Default
            ...
            await _reactNativeBleManager.default.startNotification(deviceId, target.serviceUUID, target.notifyCharacteristicUUID);
      ...
      if (foundConfig) {
        ...
        (_connectPromiseRef$cu = connectPromiseRef.current) === null || _connectPromiseRef$cu === void 0 || _connectPromiseRef$cu.resolve(peripheralInfo);
      ...
      var _connectPromiseRef$cu2;
      const formattedError = handleError(error);
      console.error(`[useBluetooth] Connection process failed for ${deviceId}:`, formattedError);
      dispatch({
      ...
      try {
        await _reactNativeBleManager.default.disconnect(deviceId);
      ...
      (_connectPromiseRef$cu2 = connectPromiseRef.current) === null || _connectPromiseRef$cu2 === void 0 || _connectPromiseRef$cu2.reject(formattedError);
      connectPromiseRef.current = null; // Clear ref
  ...
  const disconnect = (0, _react.useCallback)(async () => {
    if (state.isDisconnecting) {
      var _disconnectPromiseRef;
      console.warn('[useBluetooth] Disconnection already in progress.');
      return ((_disconnectPromiseRef = disconnectPromiseRef.current) === null || _disconnectPromiseRef === void 0 ? void 0 : _disconnectPromiseRef.promise) ?? Promise.resolve();
    }
    ...
    // Create deferred promise
    disconnectPromiseRef.current = createDeferredPromise();
    try {
      var _disconnectPromiseRef2;
      // 1. Stop Notifications
      console.info(`[useBluetooth] Stopping notifications for ${config.notifyCharacteristicUUID}...`);
      await _reactNativeBleManager.default.stopNotification(deviceId, config.serviceUUID, config.notifyCharacteristicUUID);
      ...
      }); // Optional: signal immediate success after call
      (_disconnectPromiseRef2 = disconnectPromiseRef.current) === null || _disconnectPromiseRef2 === void 0 || _disconnectPromiseRef2.resolve();
    } catch (error) {
      var _disconnectPromiseRef3;
      const formattedError = handleError(error);
      ...
      (_disconnectPromiseRef3 = disconnectPromiseRef.current) === null || _disconnectPromiseRef3 === void 0 || _disconnectPromiseRef3.reject(formattedError);
  ...
  const executeCommand = (0, _react.useCallback)(async (command, returnType, options) => {
    if (!state.connectedDevice || !state.activeDeviceConfig) {
    ...
    const config = state.activeDeviceConfig;
    const deviceId = state.connectedDevice.id;
    const commandTimeoutDuration = (options === null || options === void 0 ? void 0 : options.timeout) ?? _constants.DEFAULT_COMMAND_TIMEOUT;
    ...
    currentCommandRef.current = {
      ...
      responseBuffer: [],
      responseChunks: [],
      // Initialize empty response chunks array
    ...
    timeoutId = setTimeout(() => {
      ...
      if (((_currentCommandRef$cu = currentCommandRef.current) === null || _currentCommandRef$cu === void 0 ? void 0 : _currentCommandRef$cu.promise) === deferredPromise) {
    ...
    try {
      const commandString = command + _constants.ELM327_COMMAND_TERMINATOR;
      const commandBytes = Array.from(new TextEncoder().encode(commandString));
      if (config.writeType === 'Write') {
        await _reactNativeBleManager.default.write(deviceId, config.serviceUUID, config.writeCharacteristicUUID, commandBytes);
      } else {
        await _reactNativeBleManager.default.writeWithoutResponse(deviceId, config.serviceUUID, config.writeCharacteristicUUID, commandBytes);
      }
      ...
      var _currentCommandRef$cu2, _currentCommandRef$cu3, _currentCommandRef$cu4;
      const formattedError = handleError(error);
      console.error(`[useBluetooth] Error during command execution or processing "${command}":`, formattedError);
      if (((_currentCommandRef$cu2 = currentCommandRef.current) === null || _currentCommandRef$cu2 === void 0 ? void 0 : _currentCommandRef$cu2.promise) === deferredPromise && currentCommandRef.current.timeoutId) {
        clearTimeout(currentCommandRef.current.timeoutId);
      }
      if (state.isAwaitingResponse && ((_currentCommandRef$cu3 = currentCommandRef.current) === null || _currentCommandRef$cu3 === void 0 ? void 0 : _currentCommandRef$cu3.promise) === deferredPromise) {
        dispatch({
          type: 'COMMAND_FAILURE',
          payload: formattedError
        });
      }
      if (((_currentCommandRef$cu4 = currentCommandRef.current) === null || _currentCommandRef$cu4 === void 0 ? void 0 : _currentCommandRef$cu4.promise) === deferredPromise) {
        currentCommandRef.current = null;
      }
      throw formattedError;
    }
  }, [state.connectedDevice, state.activeDeviceConfig, state.isAwaitingResponse, dispatch, currentCommandRef]);
  const sendCommand = (0, _react.useCallback)(async (command, options) => {
    const result = await executeCommand(command, _constants.CommandReturnType.STRING, options);
    if (typeof result !== 'string') {
      throw new Error('Internal error: Expected string response but received bytes or chunks.');
    }
    return result;
  }, [executeCommand]);
  const sendCommandRaw = (0, _react.useCallback)(async (command, options) => {
    const result = await executeCommand(command, _constants.CommandReturnType.BYTES, options);
    if (!(result instanceof Uint8Array)) {
      throw new Error('Internal error: Expected byte response but received string or chunks.');
    }
    return result;
  }, [executeCommand]);

  // New function for chunked responses
  const sendCommandRawChunked = (0, _react.useCallback)(async (command, options) => {
    const result = await executeCommand(command, _constants.CommandReturnType.CHUNKED, options);
    if (typeof result === 'string' || result instanceof Uint8Array) {
      throw new Error('Internal error: Expected chunked response but received string or bytes.');
    }
    return result;
  }, [executeCommand]);
  const setStreaming = (0, _react.useCallback)(shouldStream => {
  ...
  (0, _react.useEffect)(() => {
    ...
    return () => {
      var _currentCommandRef$cu5;
      if ((_currentCommandRef$cu5 = currentCommandRef.current) !== null && _currentCommandRef$cu5 !== void 0 && _currentCommandRef$cu5.timeoutId) {
        clearTimeout(currentCommandRef.current.timeoutId);
      }
    };
  }, [currentCommandRef]);
  ...
  (0, _react.useEffect)(() => {
    if (state.isAwaitingResponse && currentCommandRef.current) {
      ...
      const promptIndex = currentCommandRef.current.responseBuffer.indexOf(_constants.ELM327_PROMPT_BYTE);
      if (promptIndex !== -1) {
        var _currentCommandRef$cu6;
        ...
        (_currentCommandRef$cu6 = currentCommandRef.current) === null || _currentCommandRef$cu6 === void 0 || _currentCommandRef$cu6.promise.resolve(responseString);
