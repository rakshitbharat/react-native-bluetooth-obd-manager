...
import { KNOWN_ELM327_TARGETS, DEFAULT_COMMAND_TIMEOUT, ELM327_COMMAND_TERMINATOR, ELM327_PROMPT_BYTE, CommandReturnType } from '../constants';
...
export const useBluetooth = () => {
   ...
   */
  const promptEnableBluetooth = useCallback(async () => {
   ...
   */
  const scanDevices = useCallback(async (scanDurationMs = 5000) => {
  ...
  const executeCommand = useCallback(async (command, returnType, options) => {
    if (!state.connectedDevice || !state.activeDeviceConfig) {
    ...
    const config = state.activeDeviceConfig;
    const deviceId = state.connectedDevice.id;
    const commandTimeoutDuration = (options === null || options === void 0 ? void 0 : options.timeout) ?? DEFAULT_COMMAND_TIMEOUT;
  ...
  const sendCommand = useCallback(async (command, options) => {
    const result = await executeCommand(command, CommandReturnType.STRING, options);
    if (typeof result !== 'string') {
      throw new Error('Internal error: Expected string response but received bytes or chunks.');
    }
    return result;
  }, [executeCommand]);
  const sendCommandRaw = useCallback(async (command, options) => {
    const result = await executeCommand(command, CommandReturnType.BYTES, options);
  ...
  const sendCommandRawChunked = useCallback(async (command, options) => {
    const result = await executeCommand(command, CommandReturnType.CHUNKED, options);
  ...
  const setStreaming = useCallback(shouldStream => {
